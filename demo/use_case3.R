...
q_genes_regions = deepblue.filter_regions(
    query_id = q_genes, 
...
experiments = deepblue.list_experiments(genome = "GRCh38",
                                        type = "signal",
...
requests = lapply(hsc_experiment_names, function (hsc_experiment) {
    ...
    q_agg = deepblue.aggregate(data_id = q_exp, 
                               ranges_id = q_genes_regions, 
                               column='VALUE')
    q_filtered = deepblue.filter_regions(query_id = q_agg, 
                                         field="@AGG.MEAN", 
                                         operation=">",
                                         value="0", type="number")
    
    req = deepblue.get_regions(
        query_id = q_filtered,
        output_format = "CHROMOSOME,START,END,@AGG.MEAN,@AGG.MAX,@AGG.MIN")
})

#download the results and save them to disk
results <- deepblue.batch_export_results(
    requests,
