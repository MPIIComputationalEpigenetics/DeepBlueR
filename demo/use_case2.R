...
message("Extract all H3k27ac from BLUEPRINT Epigenome project (for chromosome 1 only)")

message("We list and extract all samples IDs with the biosource H1-hESC from the ENCODE project.")
...
message("We list all peaks experiments that contains the previously selected samples IDS, the histone modification H3K4me3 from the ENCODE project.")
readline("press any key to continue")
...
message("and generate a list of experiments that the original file name ends with 'bed.gz'.")
readline("press any key to continue")
...
message("As we are interested in only one experiment file, we do a check if found only one experiment.")
readline("press any key to continue")
...
message("Then, we list and extract the names of the DNA Methylation experiments that contain liver or hepatocyte biosource.")
readline("press any key to continue")
...
message("We use the auxiliar function batch_export_results to download and store the request data in the target.directory")
readline("press any key to continue")
results <- deepblue.batch_export_results(requests, target.directory="liver_aggregation")

message("Some of the requests failed. You can access the problematic commands like this")
readline("press any key to continue")

attr(results, "errors")

message("This will allow you to chceck this errors individually, for example with the first error")
