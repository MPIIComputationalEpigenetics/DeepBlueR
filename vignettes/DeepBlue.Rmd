...
description of DeepBlue you may find additional material in
[DeepBlue's publication](http://dx.doi.org/10.1093/nar/gkw211).
If you find DeepBlue useful and use it in your project consider citing this
paper.
...
performed using the devtools package. All you have to do is to execute the
...
A list of all commands available by DeepBlue is provided in its
 [API page](http://deepblue.mpi-inf.mpg.de/api.php).

For reference, following are listed the most used DeepBlue commands. Note that
 each command has the prefix 'deepblue.*', e.g. deepblue.select_genes.
...
We filter the genomic regions that have the value of the column PEAK higher
than 1000.
...
The ```get_regions``` command is asynchronous. It means that the user receives a
```request_id``` and should check the status of this request. The specific
DeepBlueR package command ```download_request_data``` wait for the processing
finishes in the server, download the data, and convert to a GRanges object.
...
request_id = deepblue.get_regions(query_id=query_id_filter_length,
    output_format="CHROMOSOME,START,END,SIGNAL_VALUE,PEAK,@NAME,",
        "@BIOSOURCE,@LENGTH,@SEQUENCE")
regions = deepblue.download_request_data(request_id=request_id)
head(regions, 1)
```
...
request_id = deepblue.get_regions(query_id=overlapped,
    output_format="CHROMOSOME,START,END,@AGG.MIN,@AGG.MAX,@AGG.MEDIAN,",
        "@AGG.MEAN,@AGG.VAR,@AGG.SD,@AGG.COUNT")
regions = deepblue.download_request_data(request_id=request_id)
regions
```
...
The command ```aggregate``` aggregates the ```query_id``` regions by their column
named VALUE, using the ```cpg_islands``` regions as boundaries.
...
request_id = deepblue.get_regions(query_id=filtered_zeros,
    output_format=
    "CHROMOSOME,START,END,@CALCULATED(return math.log(value_of('@AGG.MEAN'))),
        @AGG.MEAN,@AGG.COUNT")
