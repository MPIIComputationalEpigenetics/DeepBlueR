...
## ---- echo=TRUE, eval=TRUE, warning=FALSE, message=FALSE-----------------
tataa_regions = deepblue.select_annotations(
...
request_id=deepblue.get_regions(overlapped,
    "CHROMOSOME,START,END,SIGNAL_VALUE,PEAK,@NAME,@BIOSOURCE,@LENGTH,@SEQUENCE")
regions = deepblue.download_request_data(request_id=request_id)
head(regions, 5)

## ---- echo=TRUE, eval=FALSE, warning=FALSE, message=FALSE----------------
#  q_genes = deepblue.select_genes(genes_name="RP11-34P13", gene_set="gencode v23")
#  q_filter = deepblue.filter_regions(query_id=q_genes,
...
query_id = deepblue.select_experiments (
    ...
    chromosome="chr1", start=0, end=50000000)
cpg_islands = deepblue.select_annotations(annotation_name="CpG Islands",
    genome="GRCh38", chromosome="chr1", start=0, end=50000000)
# Aggregate
overlapped = deepblue.aggregate (data_id=query_id, ranges_id=cpg_islands,
    column="VALUE" )
...
request_id = deepblue.get_regions(query_id=overlapped,
    output_format=
...
tiling_id = deepblue.tiling_regions(size=100000,
    genome="GRCh38", chromosome="chr1")

# Aggregate
overlapped = deepblue.aggregate (data_id=query_id,
...
before_flank_id = deepblue.flank(query_id=q_genes,
...
cpg_islands = deepblue.select_annotations(
    annotation="CpG Islands", genome="GRCh38", chromosome="chr1")

# Aggregate
overlapped = deepblue.aggregate(
...
filtered = deepblue.filter_regions(query_id=overlapped,
    field="@AGG.COUNT", operation=">", value="0", type="number")

# We remove all regions where the aggregation mean is zero.
filtered_zeros = deepblue.filter_regions(query_id=filtered,
    field="@AGG.MEAN", operation="!=", value="0.0", type="number")

# Retrieve the experiments data (The @NAME meta-column is used to include the
# experiment name and @BIOSOURCE for experiment's biosource
request_id = deepblue.get_regions(query_id=filtered_zeros,
    output_format=
...
cpgs = deepblue.select_annotations(
    annotation_name="Cpg Islands",
    chromosome="chr22", start=0, end=18000000, genome="hg19")

request_id = deepblue.score_matrix(
