...
In this use case we will show how DeepBlueR can be used to generate an
overview heatmap of variable positions in all BLUEPRINT DNA Methylation experiments

# Data retrieval with DeepBlueR
...
Next, we list all available BLUEPRINT DNA methylation experiments (412 files).
...
(opposed to coverage files) and retain half of the files.
...
In each of these files, there is a column that holds the beta value of a specific
site. To select this column across several files there are two possibilities.
The first one assumes that the column in question potentially has a different
...
In this case and most other cases, the same column name will apply for each
...
In the next operation, we consider that not all methylation sites in the data
will be informative for clustering the data according to their sample type,
which is the goal of this use case. We thus filter for those regions that are
part of the BLUEPRINT regulatory build, a modified version of the ENSEMBL
regulatory build (http://www.ensembl.org/info/genome/funcgen/regulatory_build.html)
...
DeepBlueR returns a query id, which we store for later.
...
DeepBlue knows several annotations that can be used to filter for informative
...
A list of all annotations currently available is given by the following command.
...
In the above step we defined a set of regions of interest that we want to
interroagate in R to, for example, cluster samples. To this end, DeepBlue can
...
to extract this information is roughly 212 x ~142 MB ~= 30 GB and thus more
than can be handled in R on most desktop computers. We next show how this score
matrix can be used to show that samples cluster by their cell type and how to
reproduce the heatmap shown as Figure 1 in the main manuscript.

# Generating a heatmap

## Meta data and colors
...
For each experiment, we collect meta data.
```{r}
experiments_info <- deepblue_info(deepblue_extract_ids(blueprint_DNA_meth))
```

All meta data is parsed to a nested R list. We refer to the DeepBlue paper for
a description of available meta data. Here, we show the meta data associated
...
More precisely, we convert the Spearman correlation, a similarity
...
This usecase is meant as a glimpse at the potential of the DeepBlueR package.
However, much can be learned and we provide ample documentation to help new
users to get started. We recommend you start by reading the vignette

```{r, eval = FALSE}
vignette("DeepBlueR")
```

Next, you should have a look at the use cases included in the R package. You
can list them with
```{r, eval=FALSE}
...
Note that the use case in this document corresponds to use case 4 in the R package.

Finally, we encourage you to try to reproduce [Python examples](http://deepblue.mpi-inf.mpg.de/examples.php) in R and to read the [DeepBlue manual](https://www.gitbook.com/book/felipealbrecht/deepblue-epigenomic-data-server-manual/details).
...
interface also allows you to select experiments in a grid like view. These can
then be downloaded to R for further analysis by simple copy and paste of the
provided code as shown in Figure 1.

![Screenshot of grid view where a selection of experiments can be exported to R.](web_interface_export.png)

# Final remarks

Should you encounter any problems with DeepBlueR, we kindly ask you to create
an issue on our [github repository](https://github.com/MPIIComputationalEpigenetics/DeepBlueR/issues).
The R code in the DeepBlueR package is under the GPLv3 license and we welcome
contributions of other developers via github. Finally, we would like to thank the
